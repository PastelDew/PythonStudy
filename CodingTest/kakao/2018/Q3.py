...
class Quiz(TestBase):
    ...
    def solution(self, relation):
        ...
        keysByColumn = self.getAllCaseOfArray([i for i in range(0, colLength)], colLength)
        keysByColumn.sort(key=lambda x: len(x))
        keyMap = {}
        for key in keysByColumn:
            items = []
            keyMap[key] = items
            cols = key.split(",")
        ...
        for key in keysByColumn:
            if not key in keyMap.keys():
                continue
            if not self.isUnique(keyMap[key]):
                continue

            cols = key.split(",")
            if len(cols) == 1:
                if self.isUnique(keyMap[key]):
                    newKeyMap.append(key)
                continue

            colLen = len(cols)
            foundNotUniqueCount = 0
            for i in range(0, len(cols)):
                colCopy = cols[:]
                del colCopy[i]
                targetKey = ",".join(colCopy)
                if not self.isUnique(keyMap[targetKey]):
                    foundNotUniqueCount += 1
            if foundNotUniqueCount == colLen:
                newKeyMap.append(key)
        return len(newKeyMap)
    ...
    def getAllCaseOfArray(self, array, length, index=0, seperator=','):
        if index >= length:
            return []
        results = [str(array[index])]

        for i in range(index + 1, length):
            newItems = []
            for item in results:
                newItems.append(item + seperator + str(i))
            results = results + newItems

        return results + self.getAllCaseOfArray(array, length, index + 1, seperator)
